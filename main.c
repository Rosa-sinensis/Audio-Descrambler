...
int main(void)
{
    ...
    for( t = 0; t < PTS; t = t + 1)
    {
        sin_value[t] = 255*(sin(2*PI*(7*t/(float)PTS))+1)/2;
    }
...
void SysTick_Handler(void)
{
    ...
    // Initializing all the constants and variables that will be used in SysTick_Handler
    static long temp_ADC = 0;
    static long temp_ADC1 = 0;
    static long temp1 = 0;
    static long temp2 = 0;
    static float x[BUFFERSIZE] = {0, 0, 0},    //input
            y[BUFFERSIZE] = {0, 0, 0},
            z[BUFFERSIZE] = {0, 0, 0};    //output
    static int n2 = 0;
    static int n1 = 2;
    static int n0 = 1;
    static int j = 0;
    ...
    temp_ADC1 = temp_ADC / 4;  //We do this because the ADC is set to use 10 bits but P2OUT is only 8 bits.

    /* Implementing the digital filter using cascaded Second-Order-Section(SOS) form */
    /* The coefficient is produced using digitalFilter program in MATLAB */
    x[n2] = temp_ADC1;
    y[n2] = 0.98663f*(x[n2] - 1.07153f*x[n1] + x[n0]) + 1.07966f*y[n1] - 0.97366f*y[n0];
    z[n2] = 0.98663f*(y[n2] - 1.07153f*y[n1] + y[n0]) + 1.03455f*z[n1] - 0.97321f*z[n0];

    /* Multiply the signal with a 7kHz sine wave to simulate the flipping of the signal at 3.5kHz */
    temp1 = (int) z[n2];
    temp2 = sin_value[j] * temp1;  // Multiplication will produce 16 bit since sin_value and temp1 are both 8 bits
    temp2 = temp2 / 256; // Divide by 2^8 to reduce the output back to 8 bits
    j++;
    if(j==PTS) {
        j=0;
    }

    /* Output the signal through P2 */
    P2OUT = temp2;
    ...
    /* How the pointers change in every iteration, it is implemented in a circular way (ring buffer) since the the element at index 2 get wrap back to the index 0 at the next iteration
     *
     * 1st Iteration:
     * x[3] = {0,   0,   0}
     *         n2  n0   n1
